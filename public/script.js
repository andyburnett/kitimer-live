// Global declaration for Cypress access and application use.
var auth;
var db; 
var firebaseApp;
...
// CRITICAL FIX: Initialization is deferred until Cypress calls this function,
// ensuring emulator setup happens first.
window.initializeClientSDK = function() {
    console.log("Cypress: Initializing Firebase Client SDK and Emulators...");
    
    // NOTE: firebaseConfig is assumed to be defined in index.html
    firebaseApp = firebase.initializeApp(window.firebaseConfig);
    auth = firebase.auth();
    db = firebase.firestore();

    // The emulator connection settings must be placed here, AFTER initialization.
    auth.useEmulator('http://localhost:9099'); 
    db.useEmulator('localhost', 8080);

    // Continue application setup flow
    ...
    return firebaseApp; // Return the app promise for Cypress to chain onto
}


// --- GLOBALS AND DOM ELEMENTS (Moved below initialization function) ---
...
const facilitatorTrigger = document.getElementById('facilitator-trigger');
...
const authStatusInfoEl = document.getElementById('auth-status-info');
const modalTimeDisplayEl = document.getElementById('modal-time-display');
...
async function updateTimerState(updates) {
    if (!projectCode) {
        // Now using alert() substitute as per instructions
        console.error("System Error: No project code defined.");
        return;
    }
    ...
    try {
        ...
        console.error("Action Failed: You may not have permission to control the timer. Please ensure you are a signed-in facilitator.", error);
    }
}
...
setButton.addEventListener('click', () => {
    ...
    if (isNaN(durationMinutes) || durationMinutes < 1) {
        console.error(`Please enter a valid duration of 1 minute or more.`);
        return;
    }
...
toggleButton.addEventListener('click', async () => {
    ...
    if (!currentTimer || currentTimer.status === 'stopped' || currentTimer.status === 'paused') {
        ...
        if (!currentTimer || currentTimer.durationSeconds == null || currentTimer.durationSeconds === 0) {
            console.error("Please set the timer duration before starting.");
            return;
        }
...
resetButton.addEventListener('click', async () => {
    ...
    if (!currentTimer || currentTimer.durationSeconds == null) {
        console.error("Cannot reset: Timer duration has not been set.");
        return;
    }
...
// NOTE: The window.initializeClientSDK() function defined earlier is what kicks off 
// the authentication chain (including the initial call to initializeAppWithAuth) 
// after the emulator hosts are set.

// Application startup is now handled by Cypress calling window.initializeClientSDK()
